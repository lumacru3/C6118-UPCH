...
Se realizó el análisis de las condiciones oceánicas frente a las costas de Perú durante el año 1990. Se utilizaron datos de temperatura superficial del mar (TSM), salinidad en UPS y fitoplancton pequeño y grande (micromol por metro cuadrado por segundo) proporcionados por el profesor del curso. Los gráficos se realizaron con Rstudio utilizando los paquetes:
...
El ecosistema peruano es altamente productivo y eso se evidencia en la abundancia de fitoplancton. Las diferencias en el tamaño del fitoplancton también significan diferencias en la disponibilidad de nutrientes y condiciones físicas como la turbulencia, y la surgencia de aguas profundas. Según la bibliografía cuando existe una menor disponibilidad de nutrientes el fitoplancton suele ser de menor tamaño ya que la absorción de estos es más eficiente, en este caso se encontrarían más dinoflagelados. Normalmente los periodos de surgencia están asociados a una mayor disponibilidad de nitratos lo que beneficiaría la proliferación de fitoplancton de mayor tamaño como las diatomeas. 
...
```
La mayor abundancia de fitoplancton se encuentra cercana a la costa peruana sostenida por la alta productividad del sistema de surgencia además de los nutrientes proporcionados por la plataforma. Aparte se puede observar la abundancia a la largo de la Zona de convergencia intertropical. 
...
```
Las diferencias entre ambos gráficos son notorias, sin embargo, se debe tener en cuenta la escala que es mucho mayor para el fitoplancton pequeño lo que demuestra su predominancia a lo largo de la costa peruana en el año 1990. El fitoplancton grande representado por las diatomeas presenta su mayor abundancia a lo largo de la costa hasta Nazca en donde disminuye, esto estaría asociado al tamaño de la plataforma que al incrementar la latitud esta disminuye. 
...
```
Los picos de abundancia de fitoplancton de tamaño reducido se encuentran asociados con los meses cálidos, esto a su vez tiene relación con la surgencia de aguas frías como se pudo ver en los gráficos de TSM. También se puede observar que el mes con la menor abundancia de fitoplancton pequeño es septiebre caracterizado por TSM frías. A continuación se compara la TSM con la abundancia de los meses de mayor y menor abundancia.

```{r , fig.width=7, fig.height=7, fig.cap=" Temperatura vs Fitopqueño ",echo=FALSE, fig.align="center"}
par(oma=c(0,0,0,2))
par(mfrow=c(2,2), mar=c(4,4,2,2))
image.map(lon, lat, sst_mes[,,3], main=paste("°C", month.name[3]))
image.map(lon, lat, sphy_mes[,,3], main=paste("Fitop pequeño µmol/m2*s",month.name[3]))
image.map(lon, lat, sst_mes[,,6], main=paste("°C",month.name[6]))
image.map(lon, lat, sphy_mes[,,6], main=paste("Fitop pequeño µmol/m2*s",month.name[6]))

```
Se evidencia una diferencia más notoria en la distribución del fitoplancton más que en la abundancia en relación a las estaciones y sus respectivas temperaturas. En el norte del Perú a la altura de Tumbes se llegan a valores de 1µmol por metro cuadrado en el mes de verano (marzo) con temperaturas promedio acercándose a los 27°C. También se observa una abundancia de 0.9 µmol en el sur del país (Tacna).
...
```
A diferencia del fitoplancton pequeño la mayor abundancia de fitoplancton de gran tamaño ocurre en los meses fríos (julio-septiembre) alcanzando los 0.7 µmol por metro cuadrado. Sin embargo no es comparable con la gran mayor abundancia de fitoplancton pequeño que alcanzó su máximo a 1 µmol por metro cuadrado. Para poder explicar con mayor precisión se deberían estudiar también la intensidad de los vientos así como la disponibilidad de nitratos y fosfatos.


## Conclusiones

* El análisis de TSM indica que el año 1990 presenta mayor cantidad de anomalías frías por lo que es considerado un año frío. No presenta anomalías suficientes como para ser considerado un año niña ya que las anomalías no sobrepasaron los 0.5° en ningún mes del año. 

* En general la salinidad presentó valores bajos a promedio, la salinidad también influye sobre la cantidad de gases disueltos en el medio acuoso ya que a mayor salinidad menor capacidad de los gases de diluirse. Se tendría que analizar los datos de Oxígeno disuelto para observar dicha relación que debería evidenciar valores normales de Oxígeno para este año.

* Los valores de abundancia de fitoplancton varían respecto al tamaño de este. Se observó una predominancia en general del fitoplancton de pequeño tamaño, esto podría verse influenciado por una baja cantidad de nutrientes lo que también se relaciona con la baja abundancia de fitoplancton grande como son las diatomeas. 

* Es necesario un análisis de los nutrientes disponibles así como de la intensidad de los vientos para completar el análisis de productividad primaria ya que ambos factores influyen bastante en la proliferación de comunidades de fitoplancton.

## Referencias

* Marco Espino et. al. IMARPE. 1990. - *"Situación de los stocks de peces demersales en el otoño 1990"*. 

* CENEPRED. 2013. - *"Escenario de Riesgos ante el Fenómeno La Niña en la costa del Perú / Perú. Centro Nacional de Estimación, Prevención y Reducción del Riesgo de Desastres. Lima: CENEPRED."*