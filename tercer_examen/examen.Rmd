...
Se realizó el análisis de las condiciones oceánicas frente a las costas de Perú durante el año 1990. Se utilizaron datos de temperatura superficial del mar (TSM), salinidad en UPS y fitoplancton pequeño y grande. Los gráficos se realizaron con Rstudio. 
...
El Perú es un país que se encuentra en una zona tropical, sin embargo no posee un clima tropical lo que se debe a la temperatura del mar que influencia la temperatura del aire y a su vez crea una atmósfera estable.
...
```{r check, fig.width=6, fig.height=6, fig.cap=" ",echo=FALSE}
...
```
En el gráfico de distribución de temperaturas en el año 1990 se observa que a lo largo de la costa peruana las temperaturas no sobrepasan los 24°C. y que a partir de la costa central las temperaturas bajan hasta los 20°C y menos. 

```{r , fig.width=4, fig.height=4, fig.cap=" ",echo=FALSE, fig.align="center"}
...
```
Cuando comparamos el año 1990 con el resto de los datos proporcionados que abarcan 46 años de nuestreo de TSM, se puede observar una anomalía negativa en el norte de la costa peruana lo que indicaría que es un año con tendencia fría. 

```{r , fig.width=8, fig.height=6, fig.cap="VARIACIÓN DE LA TEMPERATURA DURANTE TODOS LOS MESES DE 1990 ",echo=FALSE, fig.align="center"}
...
En el diagrama de anomalías a lo largo del año 1990 se puede identificar fácilmente que la mayor anomalía de temperatura se da en el mes de marzo alcanzando más de 3 grados de anomalía. Por otro lado en el mes de setiembre se encuentra la mayor anomalía negativa alcanzando temperaturas con anomalías negativas menores a -2°C.
...
El año 1990 presenta una salinidad promedio de 35 UPS, este valor está dentro de las salinidades que son consideradas normales para el agua de mar, sin embargo, para el gráfico de anomalías se observa una ligera anomalía en el promedio, es necesario analizar cada mes para localizar en qué momento del año se desarrolla dicha anomalía.
...
par(mfrow=c(3,4), mar=c(3,3,3,4)) #se coloca partición del gráfico antes de realizar el for
